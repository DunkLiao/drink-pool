...
本文說明如何將 `drink-pool` 部署到 Zeabur。此專案是 Flask + SQLite 應用，使用 `zbpack.json` 啟動，並可選擇啟用 PaddleOCR 與 OpenRouter AI 修正菜單。
...
8. 到「菜單品項」頁確認 OCR 狀態。
9. OCR 完成後，可手動修正品項。
10. 若已設定 OpenRouter，可點選「AI 修正 OCR」排入背景處理，完成後確認草稿並套用。
11. 回首頁測試送出一筆訂單。
...
PaddleOCR 會在第一次執行時下載與載入模型，冷啟動時間較長。若 Zeabur 資源不足，可先使用手動新增菜單品項。
